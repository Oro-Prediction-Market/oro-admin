...
// Default close date: the Champions League final window (early June, next year
// if we're already past June).
...
export default function UclStatMarketsPage() {
  const token = sessionStorage.getItem("admin_token")
  const api = useAdminApi(token)

  const [stats, setStats] = useState<UclStats | null>(null)
  ...
  const load = async () => {
    setErr(null)
    try {
      const res = await api.getUclStatMarketPreview()
  ...
  const create = async (stat: string) => {
    ...
    try {
      ...
      const market = await api.createUclStatMarket({ stat, closesAt, topN })
      setMsg((m) => ({
        ...m,
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: 1000 }}>
        ...
        >
          {season.started ? (
            <>
              ...
              matchday(s) played. Markets auto-create once ~2 matchdays are in;
      ...
      <StatBoardEditor league="ucl" leagueLabel="Champions League" />
    </div>
  )
}
